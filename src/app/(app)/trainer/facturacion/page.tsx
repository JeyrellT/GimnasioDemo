...
import { getMyTrainerInvoices, getMySubscription } from "@/app/actions/billing";
...
export default async function FacturacionPage() {
	...
	const [invoices, subRes] = await Promise.all([
		...
		<div className="space-y-6">
			...
			<div className="rounded-xl border border-[#F59E0B]/30 bg-[#451A03] px-4 py-3">
				<p className="text-sm text-[#FDE68A]">
					Procesamiento de pagos en sandbox. Los cobros se registran pero no se
					procesan hasta que activés{" "}
						...
						>
							<div>
								...
								<p className="text-xs text-[#71717A] mt-0.5">
									{formatDateCR(inv.periodStart, "d MMM")} –{" "}
									{formatDateCR(inv.periodEnd, "d MMM yyyy")}
								</p>
							</div>

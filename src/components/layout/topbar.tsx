...
import {
  MirrorViewSwitcher,
  type MirrorViewSwitcherState,
} from "@/app/(app)/admin/_components/mirror-view-switcher";
...
export function Topbar({
  ...
  const logoAlt = branding.businessName
    ? `Logo de ${branding.businessName}`
    : "Logo";
  ...
  return (
    <header className="relative flex h-14 items-center justify-between bg-canvas px-4 sm:px-6">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(90deg, transparent, var(--brand-primary) 50%, transparent)",
          opacity: 0.5,
        }}
      />
      ...
      >
        <span className="flex min-w-0 items-center gap-2 sm:hidden">
          {mobileLogoSrc ? (
            ...
            <img
              src={mobileLogoSrc}
              alt={logoAlt}
              className="max-h-8 max-w-[92px] shrink-0 object-contain"
            />
          ) : (
        ...
        <span className="hidden sm:inline-flex items-center gap-2">
          {desktopLogoSrc ? (
            ...
            <BlacklineFitnessLogo
              variant={branding.businessName ? "mark" : "full"}
              size={34}
            />
          )}
          {branding.businessName && (
      ...
      {mirrorSwitcher ? (
        <MirrorViewSwitcher state={mirrorSwitcher} />
      ) : (
        <DropdownMenu>

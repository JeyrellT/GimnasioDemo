...
import { PALETTE_PRESETS, HEX_COLOR_REGEX } from "@/lib/branding/presets";
...
export function BrandingSection() {
  ...
  function handleSelectPreset(presetId: string) {
    update({ paletteId: presetId, customHex: null });
    setHexInput("");
  }

  function handleApplyCustomHex() {
    if (!hexValid) {
      toast.error("Ingresá un código hex válido (ej: #FF5500).");
      return;
    }
    update({ paletteId: "custom", customHex: hexInput.toUpperCase() });
  ...
  return (
    <>
      ...
      >
        ...
        {/* Show active custom color indicator */}
        {isCustomActive && branding.customHex && (
          <div className="flex items-center gap-2 text-xs text-[#A1A1AA]">
            <div
              className="h-4 w-4 rounded-full border border-white/30"
              style={{ backgroundColor: branding.customHex }}
            />
            <span>Color activo: <span className="font-mono text-[#FAFAFA]">{branding.customHex}</span></span>
          </div>
        )}

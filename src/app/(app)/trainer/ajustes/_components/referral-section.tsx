...
// =============================================================================
// BLACKLINE FITNESS — ReferralSection
// Trainer settings page — "Referir un nuevo coach" card.
...
interface ReferralSectionProps {
  /** Pre-fetched list from the server so the initial render is not empty. */
...
// ── Date helper (client-safe, no tz lib needed for display) ───────────────────
...
export function ReferralSection({ initialReferrals }: ReferralSectionProps) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      // Refresh the referrals list optimistically from the server
      const listResult = await getMyReferrals();
      if (listResult.ok) {
        setReferrals(listResult.value);
      }
  ...
  return (
    <section className="rounded-xl border border-[#3F3F46]/60 bg-[#18181B] p-5 space-y-5">
      ...
      <p className="text-xs text-[#71717A]">
        Referí a un colega entrenador. Tu referencia será revisada por el equipo
        de Blackline Fitness.
      </p>
